...
const AddManagerForm = () => {
  ...
  const onSubmit = async (values: any) => {
    ...
    values.sellsManager.profileImg = image;

    try {
      const res = await createSellsManager({ ...values }).unwrap();

      console.log(res);
      if (res?.data?.id) {
        message.success("sellsManager added successfully");
      } else {
  ...
  return (
    <div>
      <h1 className="text-center my-1 font-bold text-2xl">Add Manager</h1>
      <div>
